...
export default function RefreshJwtToken() {
  ...
  function HandleReject(error) {
    MyNotification("red", "שגיאה בחיבור משתמש", `${error.msg ? error.msg : "חיבור לשרת נכשל"}`)
  ...
  const refreshTokenUser = (refeshToken) => {
    ...
    function HandleResponse(data) {
      if (data.access_token) {
        ...
        MyNotification("red", "שגיאה בשליפת נתונים", ` שגיאה :`)
        redirect('/login/') // redicrect to Login Screen if can't find user data
      }
    ...
    FatchDataApi('refresh_token_user', 'POST', refeshToken, HandleResponse, { onReject: HandleReject, errorMsgTitle: "שגיאה בחיבור משתמש" })
