...
export default function EventDetails({ event, className = 'EventDetails', showInfo = true ,disabledFocus=false}) {
  ...
  const [eventsToHandle, setEventsToHandle] = useState((event?.event_id) ? [event.event_id] : [])
  ...
  const gridType = (event.status === 2) ? 1 :
    ...
    <div className={`${customClass} grid${gridType}`}>
      ...
      {(event.status !== 2) && <><HydrantOtherEvents focusedEvent={event} eventHandle={eventHandle} setEventsToHandle={setEventsToHandle} disabledFocus={disabledFocus} /> <hr /> </>}

      {/**********************  טיפולים לאירוע\פרטי אירוע ************************/}
      {(showInfo) ?
        <EventInfo focusedEvent={event} invoke={invoke} /> :
        (event.status !== 0) && <EventHistoryHandles event_id={event.event_id} invoke={invoke} />}
