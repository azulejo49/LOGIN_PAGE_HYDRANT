      ...
      if (temp) {
        ReactDOM.render(
          <FormGroup >
            ...
            <MySwitch label='הצגת תת תאגיד' checked={heritageVisible} toggle={() => { setHeritageVisible(prev => !prev) }} />
          </FormGroup>, temp)
        buttonContainer.removeEventListener("DOMNodeInserted", handleAddElement)
      }
    }
...
// Update hydrant visibility 
  useEffect(() => {
    hydrantsMarkers.current?.forEach(marker =>
      marker.setMap(((marker.status === 1 || disabledVisible) && (marker.event || emptyVisible)
  ...
  function resizeMarkersByZoom(zoomLevel) {  
    if (zoomLevel > 12)
      hydrantsMarkers.current?.forEach(marker => { //update all Markers Visibilty & Size
        ...
        let size = (zoomLevel - 10) * 15 - 25; //Calculates new markers size
        marker.icon.scaledSize.height = size; //set new marker height
        marker.icon.scaledSize.width = size;  //set new marker width
      });
    else
      hydrantsMarkers.current?.forEach(marker => { marker.visible = false; });  //hide all markers on zoom level too far
  }
  ...
  function hydrantBuilder(hydrant) {
    ...
    const size = (window.globalMap.getZoom() - 10) * 15 - 25; //Calculates new markers size
    const event = getEvent(hydrant.hydrant_id);
    const trig = `trig${(!event) ? 0 : (event?.status === 1) ? "open" : event?.trigger_id}`; //gets hydrant Event Icon
    if (!hydrantMarker) {
      const marker = new window.google.maps.Marker({         //create markkers with thus settings :
        optimized: true,
        position: { lat: hydrant.lat, lng: hydrant.lng },
        map:
          ((hydrant.status === 1 || disabledVisible) && (event || emptyVisible)
            && (hydrant.customer_id === userInfo?.customer_id || heritageVisible)) ? window.globalMap : null,
        event: event,
      ...
      // window.google.maps.event.clearInstanceListeners(hydrantsMarkers.current[hydrantMarkerIdx])                 
      hydrantMarker.event = event;
      // hydrantsMarkers.current[hydrantMarkerIdx].setmap
  ...
  function smoothZoomToCord(map, cord, startZoom) {
    if (map && map.getBounds()?.contains(cord)) { //after finish zooming out targat is in bounds
      ...
      var z = window.google.maps.event.addListener(map, 'zoom_changed', event => { //recursive call back to timeout zoom
        // console.log('zoomChanged event:',z)
